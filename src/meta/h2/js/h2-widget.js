...
class H2Widget extends Polymer.mixinBehaviors([BaseBehavior], H2WidgetBase) {
  ...
  static get observers() {
    return ['__fieldMetaChange(fieldMeta)', '__valueChange(value)'];
  }
  ...
  __renderElement(TagName, props) {
    const reactObj = ReactDOM.render(React.createElement(TagName, null), this.$.slot);
  ...
  __fieldMetaChange() {
    if (this.editable) {
      ...
      ReactDOM.render(React.createElement(TagName, {
      ...
      ReactDOM.render(React.createElement(TagName, { element: this.render(this.context, this.fieldMeta) }), this.$.slot);
    }
  }
